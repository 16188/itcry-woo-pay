<?php
/**
 * ITCRY WOOPAY - Easypay Alipay Gateway
 */
if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

class ITCRY_WOOPAY_Gateway_Easypay_ZFB extends ITCRY_WOOPAY_Abstract_Gateway {

    /**
     * 构造函数
     */
    public function __construct() {
        $this->id                 = 'itcry_woo_pay_easypay_zfb';
        $this->gateway_type       = 'easypay';
        $this->icon               = ITCRY_WOOPAY_URL . 'assets/logo/zfb-logo.jpg';
        $this->method_title       = __( '易支付 - 支付宝', 'itcry-woo-pay' );
        $this->method_description = __( '使用易支付聚合接口进行支付宝付款。', 'itcry-woo-pay' );
